...
let inline parseElement element =
    match element with
    | '*' -> Some(Wall)
    | ' ' -> Some(Path)
    | _   -> None
...
let inline parseLineWithEntrance (line: string) (row: int) =
...
let inline parseLine (line: string) (row: int) =
    line
    |> Seq.mapi
        (fun col c ->
            let parsed_element = parseElement c
...
let inline manhattanDist (a: int*int) (b: int*int) =
...
let aStar (maze: Map<int*int, MazeElement>) (start: int*int) (goal: int*int) =
    ...
    let inline neighbors (x: int*int) =
        seq {
            yield fst x-1, snd x
            yield fst x, snd x-1
            yield fst x, snd x+1
